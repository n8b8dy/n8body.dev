...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ name, id, className, errors, label, icon, ...props }, ref) => {
    return (
      <div>
        ...
        <div className={cn('relative')}>
          <div className={cn('absolute top-0 left-0 opacity-50')}>{icon}</div>
          <input
            name={name}
            id={id}
            className={cn(
        ...
        <ErrorMessage
          ...
          render={({ message }) => (
            <div className={cn('mt-1 relative text-sm text-red-600 italic')}>
              <IoWarningOutline
                className={cn('top-[-1.5px] relative mr-1 inline text-[1.1em]')}
              />
              <span>{message}</span>
            </div>
          )}
